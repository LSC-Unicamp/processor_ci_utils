...
def create_cocotb_makefile(processor_name: str, config_folder: str, output_dir: str, cocotb_name: str = 'cocotb_labeler'):
    ...
    if any(file.endswith('.v') for file in sim_files):
        ...
        language = None
        logging.error("No valid source files found.")
    ...
    with open(makefile_path, 'a', encoding='utf-8') as makefile:
        if language == 'verilog':
            ...
            for dirs in inc_dir:
                makefile.write(f'SYSTEMVERILOG_INCLUDE_DIRS = cores/{processor_name}/{dirs}\n')
            for file in sim_files:
                makefile.write(f'SYSTEMVERILOG_SOURCES += cores/{processor_name}/{file}\n')
        elif language == 'vhdl':
...
#            makefile.write(f'COMPILE_ARGS ?= -g{language_version}\n')
#            for dirs in inc_dir:
#                makefile.write(f'VERILOG_INCLUDE_DIRS = cores/{processor_name}/{processor_name}/{dirs}\n')
#            for file in sim_files:
#                makefile.write(f'VERILOG_SOURCES += cores/{processor_name}/{processor_name}/{file}\n')
#        elif language == 'systemverilog':
#            makefile.write('SIM ?= verilator\n')
#            makefile.write('TOPLEVEL_LANG ?= systemverilog\n')
#            makefile.write(f'COMPILE_ARGS ?= --language 1800-{language_version}\n')
#            for dirs in inc_dir:
#                makefile.write(f'SYSTEMVERILOG_INCLUDE_DIRS = cores/{processor_name}/{processor_name}/{dirs}\n')
#            for file in sim_files:
#                makefile.write(f'SYSTEMVERILOG_SOURCES += cores/{processor_name}/{processor_name}/{file}\n')
#        elif language == 'vhdl':
#            makefile.write('SIM ?= ghdl\n')
#            makefile.write('TOPLEVEL_LANG ?= vhdl\n')
#            makefile.write(f'COMPILE_ARGS ?= --std={language_version}\n')
#            for dirs in inc_dir:
#                makefile.write(f'VHDL_INCLUDE_DIRS = cores/{processor_name}/{processor_name}/{dirs}\n')
#            for file in sim_files:
#                makefile.write(f'VHDL_SOURCES += cores/{processor_name}/{processor_name}/{file}\n')
#        makefile.write(f'TOPLEVEL = {top_module}\n')
#        makefile.write(f'MODULE = {cocotb_name}\n')
#        makefile.write(f'OUTPUT_DIR = {output_dir}/{processor_name}\n')
#        makefile.write('export OUTPUT_DIR\n')
